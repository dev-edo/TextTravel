...
def infer(number,message):
	...
	if number = db_phone: #check to see what operator we are using as defined in 'operator'
		...
		if (operator = None) and (db_phone == number):
			...
			if (db_origin == None) and (db_destination != None): #if there is nothing in origin and we have a dest for this number
		...
		send_text.text(number,'Your message is bullshit and we cannot proccess it.')
